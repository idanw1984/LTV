...
CATGORICAL_FEATURES =['c_seg_id_4', 'c_seg_id_1', 'c_seg_id_3', 'c_seg_id_2', 'netsource_id_4', 'netsource_id_2', 'netsource_id_1', 'netsource_id_3']
...
for app in apps:
    ...
    prediction_matrix[('data_14d', app)] = df_for_prediction[ (df_for_prediction['vetek_14d']==1) & (df_for_prediction['bundle_id'] == app)]
    prediction_matrix[('data_30d', app)] = df_for_prediction[((df_for_prediction['vetek_30d']) == 1) & (df_for_prediction['bundle_id'] == app)]
...
for i,day in zip(range(len(DAYS)),DAYS):
    for app in apps:
        ...
        for cat_feature in CATGORICAL_FEATURES:
            list_col.append(cat_feature)
...
for app in apps:
    for day in DAYS:
        ...
        prediction_matrix[('data_' + day, app)] = pd.get_dummies(prediction_matrix[('data_'+day, app)], columns=['cluster','c_seg_id','netsource_id'])

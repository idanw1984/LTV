...
dict_std = {}
for i in  range(len(apps)):
     dict_std[('7d', apps[i])] = dict_scaler[('scaler_7d', apps[i])].fit_transform(df_test[(df_test['vetek_7d']==1) & (df_test['bundle_id']==apps[i])][COLUMNS_FOR_KMEANS_D7])
     dict_std[('14d', apps[i])] = dict_scaler[('scaler_14d', apps[i])].fit_transform(df_test[(df_test['vetek_14d'] == 1) & (df_test['bundle_id'] == apps[i])][COLUMNS_FOR_KMEANS_D14])
     dict_std[('30d', apps[i])] = dict_scaler[('scaler_30d', apps[i])].fit_transform(df_test[(df_test['vetek_30d'] == 1) & (df_test['bundle_id'] == apps[i])][COLUMNS_FOR_KMEANS_D30])
...
for i in  range(len(apps)):
    for day in DAYS:
        ...
        kmeans_dict[('scaler_'+day, apps[i])] = dict_scaler[('scaler_'+day,apps[i])]
...
for app in apps:
    for day in DAYS:
        ...
        Xgboost_model_list[('xg_reg_Train_model',day,app)] = XGBRegressor(n_jobs=8, objective='reg:squarederror', scoring='mae', bootstrap=True)
        Xgboost_model_list[('xg_grid_Train_model', day, app)] = GridSearchCV(estimator=Xgboost_model_list[('xg_reg_Train_model',day,app)], param_grid=PARAMS, cv=CV, refit=True, n_jobs=8)
